...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_medication_logs_user_status_time 
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_prescriptions_clinician_status 
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_notification_history_status_retry 
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_engagement_scores_metrics 
ON public.patient_engagement_scores USING GIN (metrics);

-- Notification history metadata
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_notification_history_metadata 
ON public.notification_history USING GIN (metadata);

-- Security events metadata for compliance queries
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_security_events_metadata 
ON public.security_events USING GIN (metadata);

-- Audit log details for forensic queries
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_audit_log_details 
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_prescriptions_pending 
ON public.prescriptions (pharmacy_id, created_at DESC) 
WHERE status = 'pending';

-- Unread clinician messages
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_clinician_messages_unread 
ON public.clinician_messages (patient_user_id, created_at DESC) 
WHERE is_read = false;

-- Active controlled drugs with low stock
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_controlled_drugs_low_stock 
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_org_subscriptions_status 
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_login_attempts_email_time 
ON public.login_attempts (email, created_at DESC);

-- Account lockouts active check
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_account_lockouts_active 
ON public.account_lockouts (email, locked_until) 
WHERE unlocked_at IS NULL;

-- Data access log for compliance audits
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_data_access_log_user_time 
ON public.data_access_log (user_id, created_at DESC);

CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_data_access_log_patient_time 
ON public.data_access_log (patient_id, created_at DESC) 
WHERE patient_id IS NOT NULL;

-- Security events by type and severity
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_security_events_type_severity 
ON public.security_events (event_type, severity, created_at DESC);

-- 6. Caregiver relationship indexes
-- =====================================================

CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_caregiver_invitations_patient 
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_medication_availability_pharmacy 
ON public.medication_availability (pharmacy_id, medication_name) 
WHERE is_available = true;

-- Drug transfers by status
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_drug_transfers_status 
ON public.drug_transfers (status, requested_at DESC);

-- 8. Lab results indexes
-- =====================================================

CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_lab_results_user_category 
ON public.lab_results (user_id, category, ordered_at DESC);

CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_lab_results_abnormal 
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_video_rooms_status 
...
CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_org_invoices_status 
ON public.organization_invoices (organization_id, status, created_at DESC);

CREATE INDEX IF NOT EXISTS IF NOT EXISTS idx_billing_events_org_type 
