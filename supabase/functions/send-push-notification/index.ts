  ...
  payload: {
    validate: (value: unknown) => {
      ...
      if (p.title.length > 100) {
        return { success: false, error: "payload.title must be 100 characters or less" } as const;
      }
      if (p.body.length > 500) {
        return { success: false, error: "payload.body must be 500 characters or less" } as const;
      }
...
serve(withSentry(FUNCTION_NAME, async (req: Request): Promise<Response> => {
  ...
  if (!vapidPublicKey || !vapidPrivateKey) {
    console.error(`[${FUNCTION_NAME}] VAPID keys not configured`);
    return new Response(
  ...
  if (subError) {
    console.error(`[${FUNCTION_NAME}] Error fetching subscriptions:`, subError);
  ...
  for (const sub of subscriptions) {
    ...
    try {
      ...
      failed++;
      console.error(`[${FUNCTION_NAME}] Push send failed:`, err);
