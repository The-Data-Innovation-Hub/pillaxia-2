...
import { MedicationDose, getMedicationFromDose, getScheduleFromDose } from "../../medications/upcomingDoses.ts";
 ...
 */
function buildMedicationRows(doses: MedicationDose[]): string {
  return doses.map((dose) => {
    const med = getMedicationFromDose(dose);
    const schedule = getScheduleFromDose(dose);
 ...
 */
export function generateMedicationReminderHtml(data: MedicationReminderData): string {
 ...
 */
export function generateMedicationReminderSubject(doseCount: number): string {
