...
import {
  ...
  listPolypharmacyWarnings,
  updatePolypharmacyWarning,
...
export function PolypharmacyWarningsCard() {
  ...
  const { data: warningsWithPatients, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const warnings = await listPolypharmacyWarnings(patientIds);
      const sorted = [...warnings].sort(
        (a, b) => ((b.medication_count as number) ?? 0) - ((a.medication_count as number) ?? 0)
      );
      const ids = [...new Set(sorted.map((w) => w.patient_user_id as string))];
      const profilesList = ids.length ? await listProfilesByUserIds(ids) : [];
      ...
      return sorted.map((w) => ({
        ...w,
        _patient: profileMap.get(w.patient_user_id as string) ?? null,
      })) as Array<PolypharmacyWarning & { _patient: PatientProfile | null }>;
    },
    enabled: !!user,
  });

  const warnings = warningsWithPatients ?? [];

  const acknowledgeMutation = useMutation({
    mutationFn: async (warningId: string) => {
      await updatePolypharmacyWarning(warningId, {
  ...
  const getPatientName = (w: (typeof warnings)[0]) => {
    const patient = "_patient" in w ? (w as { _patient: PatientProfile | null })._patient : null;
    return patient
      ? `${patient.first_name || ""} ${patient.last_name || ""}`.trim() || "Unknown"
      : "Loading...";
  };

  const unacknowledgedCount = warnings.filter((w) => !w.is_acknowledged).length;
              ...
              >
                <div className="flex items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium">{getPatientName(warning)}</p>
